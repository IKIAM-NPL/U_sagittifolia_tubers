...
```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}
POS_feat_name <- readxl::read_excel("POS_Iden_Comp_2.xlsx", 1)
...
ggplot(POS_loadings, aes(PC1, PC2)) + 
  ...
  theme_classic() + 
  geom_point(data = POS_compouds_all, aes(shape = Level, color = Level), size = 3) +
...
```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}
met_hetmap <- readxl::read_excel("POS_Data_Pablo_to_Hetmap.xlsx", 3)
...
```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE, results='hide'}
png(filename ="LC-MS2-Pos-HeatMap.png", width = 10, height = 6, units = "in", pointsize = 12, res=300)
